<div class="card">
	...
	<div class="card-body p-2">
		...
		<div style="display:<?php if (!empty($selectedControllers)) {
								echo 'none';
							} ?>" id="permissionMatrix">
				...
				if (!empty($row['actions'])) {
					...
					foreach ($row['actions'] as $action) {
						...
						foreach ($selectedUserGroups as $group) {
							...
							echo "<td style='min-width:100px;' class='text-center p-0' title='" . $group['id'] . " (Click to change permission)'>";
							$img = $no;
	...
	$(function() {
		...
		$(document).on('click', '.permission_row .change_permission', function(e) {
			...
			$.ajax({
				...
				url: url,
				headers: {
					"X-CSRF-Token": "<?php echo $this->request->getCookie('csrfToken'); ?>"
				}
			});
		});
